...
export default function Uye({ children=<></>, setToplamBegeni, begeni=0, id=0, gorsel="https://i.pravatar.cc/150?img=0", adsoyad="Belirtilmemiş", eposta="Belirtilmemiş" }) {
  const [begeniSayisi, setBegeniSayisi] = useState(begeni);

  function begeniArttir() {
    setBegeniSayisi(mevcutDeger => mevcutDeger + 1);
    setToplamBegeni(mevcutDeger => mevcutDeger + 1);
  }
  ...
  return (
    <div className="flex flex-col items-center bg-white p-4 rounded-lg shadow-md">
      ...
      <div className="flex gap-2">
        <Button onClick={ ()=> begeniArttir()  }>+</Button>
        <Button onClick={ ()=> begeniAzalt()  }>-</Button>
      </div>
    </div>
  );
}
