...
function Uyeler() {
  const [toplamBegeni, setToplamBegeni] = useState(()=> uyeler.reduce((toplam, uye) => toplam + uye.begeni, 0));
  
  return (
    <div>
      <h2>Üyeler - Toplam Beğeni: {toplamBegeni}</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {
            uyeler.map( (deger, indeks) => <Uye setToplamBegeni={setToplamBegeni} begeni={deger.begeni} id={deger.id} key={deger.id} gorsel={deger.gorsel} adsoyad={deger.adsoyad} eposta={deger.eposta} />)
        }
      </div>
    </div>
  );
